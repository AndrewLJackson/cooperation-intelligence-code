...
%% set properties of the experiment - NIAMH! Mess with this!
% these two parameters, r_negbin and p_negbin control the number of rounds
% in each generation. So... in one generation, all individuals play each
% other at the prisoners dilemma for "rounds" times.
r_negbin = 500;
p_negbin = 0.9; % NB must be between 0 and 1!!

% the expected (mean) number of rounds
expected_rounds =  1 + r_negbin .* (1-p_negbin) ./ (p_negbin);

% the variation in rounds
var_rounds = r_negbin .* (1-p_negbin) ./ (p_negbin) .^ 2;

% a quick visualisation of what these numbers mean
figure(101)
hist(nbinrnd(r_negbin, p_negbin, 1, 10 .^ 3) + 1, 30)
...
PD=[1,4; 0,3]; % payoff matrix
mem_cost = 0.01;

%% set GA properties

pop = 50; % population size
generations = 10000; % number of generations to run the GA for
...
%% run GA
Rounds = nbinrnd(r_negbin, p_negbin,(sum(1:pop).*generations),1) + 1; 
             
     
%Rounds = zeros((sum(1:pop).*generations),1);
%Rounds(:,:) = 50;
C = 1;
...
for j = 1:generations
    ...
    for i = 1:pop
        for g = 1:rep*length(pp)
            ...
            memout = 0;
            for k = 1:20
                [X,memout] = playnet(oppscore, myscore,...
                                     inputweights(:,:,i),...
                                     memweights(:,i), ...
                                     interweights(:,:,i), ...
                                     thresholds1(i,:), ...
                                     thresholds2(i), ...
                                     memout, firstmove(i,1), c, ...
                                     structure(i,:),memyn(i,:));
    ...
    firstmove = newfirst;
    changestruc = (rand(size(structure))<(mutrate./5)).*(2.*round(rand(size(structure)))-1);
    changestruc((structure(:,1)+changestruc(:,1))-(structure(:,2)+changestruc(:,2))<0,2) = 0;
    changestruc((structure+changestruc)<0) = 0;
    changestruc((structure+changestruc)>10) = 0;
...
yyy = zeros(6,generations);
for i=1:generations
    yyy(:,i) = histc(nearest_strat(:,i),1:6);
end
...
direction = zeros(generations,1);

for i = 2:generations
    direction(i) = coopmean(i) - coopmean(i-1);
end


yi = linspace(min(direction),max(direction),200);
xi = linspace(min(coopmean),max(coopmean),200);
[XI,YI]=meshgrid(xi,yi);
[X,Y,Z]=griddata(coopmean,direction,correlations,XI,YI);
%contourf(X,Y,Z)

r=ksrmv([X(:) Y(:)],Z(:),[0.1 0.05]);
L=Z;
Lr=Z;
Lr(:)=r.f;
%surf(Lr)
...
hold all
colormap(flipud([(0:0.01:0.8)',(0:0.01:0.8)',(0:0.01:0.8)']))%colormap(flipud(gray))
xlabel('Frequency of cooperation')
ylabel('Change in cooperation')
h=colorbar('FontSize',16,'linewidth',1,'TickLength',[.02 0.5],'FontName','Arial');
set(get(h,'ylabel'),'String', 'Selection for intelligence','FontName','Arial','FontSize',16);

% Xi = linspace(min(resint),max(resint),200);
% Yi = interp1q(resint,correlations,Xi');
% r=ksrmv(Xi(:),Yi(:),[0.1]);

r=ksrmv(resint,correlations,[1]);
subplot(3,1,3)
plot(resint,r.f,'ok')
set(gca,'linewidth',1,'XMinorTick','on','YMinorTick','on','FontName','Arial','FontSize',16,'TickLength',[.02 1]);
xlim([min(resint) max(resint)])
xlabel('Phenotypic diversity')
ylabel('Selection for intelligence')

r=ksrmv(dist2centre',correlations,[1]);
subplot(3,1,2)
plot(dist2centre',r.f,'ok')
set(gca,'linewidth',1,'XMinorTick','on','YMinorTick','on','FontName','Arial','FontSize',16,'TickLength',[.02 1]);
xlim([min(dist2centre) max(dist2centre)])
